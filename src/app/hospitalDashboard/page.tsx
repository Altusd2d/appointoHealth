...
function SettingsPanel({ hospital }: { hospital: unknown }) {
  ...
  const sendDataToAdmin = async (
    ...
    try {
      const res = await fetch("/api/hospital/changeData", {
        method: "POST",
        headers: {
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-slate-200 p-4">
          ...
          <button
            ...
            onClick={() => {
              sendDataToAdmin(hos, email, num, loc);
            }}>
            Save Settings
          </button>
        </div>
...
function AppointmentsPanel({ hospital }: { hospital: unknown }) {
  ...
  const fetchAppointment = async (searchId: string) => {
    try {
      // console.log()
      const loginQuery = await fetch("/api/hospital/searchAppoinment", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const waitingApp = async (id: string, status: string) => {
    try {
      console.log("id", id);
      const loginQuery = await fetch("/api/hospital/changeStatus", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const fetchAppoiinments = async () => {
    const loginQuery = await fetch("/api/hospital/Allappoinments", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
        ...
        <article
          ...
          <div className="p-2.5">
            ...
            <div className="rounded-md border border-slate-300 bg-[#efefef] p-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs">{appointment.doctor.specialist}</p>
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-13">
              <button
                onClick={() => waitingApp(appointment.id, "booked")}
                className="rounded-md bg-[#f4d632] px-3 py-1 text-xs font-bold text-white">
                Waiting
              </button>

              <button className="rounded-md bg-[#f50000] px-3 py-1 text-xs font-bold text-white">
                Reschedule
              </button>
...
function DashboardPanel({ hospital }: { hospital: unknown }) {
  ...
  const waitingapp = hos.filter((item) => item.status === "waiting").length;
...
export default function HospitalDashboardPage() {
  ...
  const CheckLogin = async () => {
    ...
    const loginQuery = await fetch("/api/services/login", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const panel = useMemo(() => {
    ...
    if (activeTab === "Settings") return <SettingsPanel hospital={hospital} />;

    if (activeTab === "Billing") return <BillingPanel />;

    return <SimplePanel title={activeTab} />;
  }, [activeTab, hospital]);

  const verifyOTP = async (otp: string, gmail: string): Promise<boolean> => {
    try {
      const res = await fetch("/api/services/verifyOtp", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const changePassword = async (gmail: string, password: string) => {
    try {
      const res = await fetch("/api/services/UpdatePassword", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      <>
        <div className="flex min-h-screen flex-col md:flex-row">
          <aside className="w-full bg-[#141821] text-white md:w-64 md:flex-shrink-0">
            <div className="border-b border-white/10 px-6 py-6 text-2xl font-semibold tracking-tight">
              Appointo Health
            </div>
          ...
          <section className="min-w-0 flex-1 p-4 md:p-6 lg:p-8">
            <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-base text-slate-600">Welcome back</p>
              </div>
              <div className="flex items-center gap-3">
...
function SimplePanel({ title }: { title: string }) {
  ...
  const openDoctor = (doctor: Doctor) => {
    ...
    try {
      ...
      setAvailability(doctorAvailability || defaultAvailability);
    } catch (err) {
      console.log(err);
  ...
  const fetchDoctors = async () => {
    try {
      ...
      const res = await fetch("/api/hospital/getDoctors", {
        method: "GET",
      });
  ...
  if (selectedDoctor) {
    ...
    const toggleSlot = (slotIndex: number) => {
      const actualIndex = selectedPeriod === "AM" ? slotIndex : slotIndex + 24;

      setAvailability((prev) => ({
        ...prev,
        [selectedDay]: prev[selectedDay as keyof typeof prev].map(
          (value, index) =>
            index === actualIndex ? (value === 1 ? 0 : 1) : value,
        ),
      }));
    };
    ...
    const saveAvailability = async () => {
      try {
        ...
        const res = await fetch("/api/hospital/ControlSlots", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          <div className="mx-auto mt-12 max-w-6xl">
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {displaySlots.map((slot, index) => {
                const actualIndex =
                  selectedPeriod === "AM" ? index : index + 24;

                const isSelected = currentDaySlots[actualIndex] === 1;
...
function BillingPanel() {
  ...
  const totalAmount = TodayBilling.reduce(
    (sum, appointment) => sum + Number(appointment.payment),
    0,
  );
    ...
    <>
      <div className="min-h-screen bg-slate-100 p-6 flex justify-center items-start">
        <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6">
            ...
            <p className="text-blue-100 mt-1">Hospital Billing Report</p>
          </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-8 py-6">
          ...
          <p className="text-emerald-100">June 2026 Overview</p>
        </div>
