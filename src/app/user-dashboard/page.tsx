...
export default function UserDashboardPage() {
  ...
  return (
    <main className="min-h-screen  px-4 py-8 md:px-10">
      <section className="mx-auto max-w-[860px] rounded-[8px] p-5 shadow-[0_0_0_1px_rgba(0,0,0,0.05)] md:p-8">
        <div className="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div className="flex items-center gap-4 md:gap-6">
            ...
            <div className="space-y-2 text-[22px] leading-tight text-[#191919]">
              <p>
              ...
              <p>
                <span className="font-semibold">Gmail:</span> Prashanthpathigari@gmail.com
              </p>
          ...
          <button className="h-7 w-10 rounded-sm bg-[#0f62c3] text-[9px] font-semibold text-white">Edit</button>
        </div>

        <div className="mt-10 rounded-[8px] bg-white/40 p-4 md:p-6">
          <div className="flex rounded-full bg-[#e6e6e6] p-1">
            <button
              onClick={() => setActiveTab("present")}
              className={`h-11 w-1/2 rounded-full text-[20px] font-semibold ${
                activeTab === "present" ? "bg-[#073766] text-white" : "text-[#1b1b1b]"
              }`}
            >
              Present Appointment
            </button>
            <button
              ...
              className={`h-11 w-1/2 rounded-full text-[20px] font-semibold ${
                activeTab === "past" ? "bg-[#073766] text-white" : "text-[#1b1b1b]"
              }`}
            >
              Past Appointment
            </button>
          ...
          {activeTab === "present" ? (
            <div className="mt-8 space-y-6">
              {presentAppointments.map((appointment, index) => (
                <article key={`${appointment.id}-${index}`} className="rounded-[4px] bg-[#dfdfdf] pb-4 shadow-[0_1px_3px_rgba(0,0,0,0.2)]">
                  <div className="grid grid-cols-2 border-b border-[#cdcdcd] text-center text-[18px] font-semibold text-[#1d1d1d]">
                    <p className="border-r border-[#cdcdcd] py-2">
                      ID:<span className="text-[#034b85]">{appointment.id}</span>
                    </p>
                    <p className="py-2">
                      ON:<span className="font-normal">{appointment.dateTime}</span>
                    </p>
                  </div>

                  <div className="px-4 pt-3">
                    <div className="flex items-center justify-center gap-5">
                      <Image src={appointment.logo} alt="Hospital logo" width={100} height={32} className="h-8 w-auto object-contain rounded-full" />
                    ...
                    <div className="mx-auto mt-3 flex max-w-[560px] items-center gap-4 rounded-[8px] border border-[#c8c8c8] bg-[#f6f6f6] p-3 shadow">
                      ...
                      <div className="text-[11px] text-[#222]">
                        <p className="text-[16px] text-[#2c6fd1]">{appointment.doctor}</p>
            ...
            <div className="mt-8 space-y-6">
              {pastAppointments.map((appointment, index) => (
                <article key={`${appointment.id}-past-${index}`} className="rounded-[4px] bg-[#dfdfdf] pb-4 shadow-[0_1px_3px_rgba(0,0,0,0.2)]">
                  <div className="grid grid-cols-2 border-b border-[#cdcdcd] text-center text-[18px] font-semibold text-[#1d1d1d]">
                    <p className="border-r border-[#cdcdcd] py-2">
                      ID:<span className="text-[#034b85]">{appointment.id}</span>
                    </p>
                  ...
                  <div className="px-4 pt-3">
                    <div className="flex items-center justify-between">
                      <Image src={appointment.logo} alt="Hospital logo" width={100} height={32} className="h-8 w-auto object-contain" />
                      <h3 className="px-2 text-center text-[30px] font-semibold leading-[0.95] text-[#111111] md:text-[23px]">
                        {appointment.hospital}
                      </h3>
                      {/* <button className="h-8 rounded border border-[#8a8a8a] bg-white px-3 text-[14px] font-semibold text-[#0b2a4d] shadow">
